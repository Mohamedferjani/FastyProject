...
public class ServiceFacture implements InterfaceFacture<Facture> {
    ...
    public void ajouterFacture(Facture p) {
     try {
    String req = "INSERT INTO `facture` (`id_facture`, `montant`, `id_prop`, `id_client`, `id_produit` ) VALUES (?, ?, ?, ?, ?)";
    PreparedStatement pstmt = connect.prepareStatement(req);
    pstmt.setInt(1, p.getId_facture());
    pstmt.setString(2, p.getMontant());
    pstmt.setInt(3, p.getId_prop().getId_user());
    pstmt.setInt(4, p.getId_client().getId_user());
    pstmt.setInt(5, p.getId_produit());
    pstmt.executeUpdate();
    ...
    public void modifierFacture(Facture p) {
        try {
            String req = "UPDATE `facture` SET `montant`= '" + p.getMontant() + "', `id_prop`= '" + p.getId_prop().getId_user() + "', `id_client`= '" + p.getId_client().getId_user() + "' ,`id_produit`= '" + p.getId_produit() + "' WHERE id_facture = '" + p.getId_facture() + "'";
            Statement st = connect.createStatement();
            st.executeUpdate(req);
    ...
    public List<Facture> getAllFacture() {
        ...
        try {
            ...
            while (rs.next()) {
                User u1 = su.getOneById(rs.getInt(3));
                User u2 = su.getOneById( rs.getInt(4));
                Facture f = new Facture(rs.getInt(1), rs.getString("montant"),u1, u2, rs.getInt(5));
                list.add(f);
            }
        } catch (SQLException ex) {
            System.out.println(ex.getMessage());
        ...
        try {
            ...
            while (rs.next()) {
                if (rs.getInt(1) == id) {
                     User u1 = su.getOneById(rs.getInt(3));
                User u2 = su.getOneById( rs.getInt(4));
                    f = new Facture(rs.getInt(1), rs.getString("montant"), u1, u2, rs.getInt(5));
                }
...
// create a cell for the image
            Image m = Image.getInstance("C:/Users/midot/OneDrive/Documents/NetBeansProjects/Integration/src/img/logo.png");
            ...
            table.addCell(new IServiceUser().getUserFullNameById(facture.getId_prop().getId_user())); //  
            table.addCell(new IServiceUser().getUserFullNameById(facture.getId_client().getId_user()));
