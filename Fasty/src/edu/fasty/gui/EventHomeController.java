...
public class EventHomeController implements Initializable {
    ...
    @FXML
    private void AdminEvent(ActionEvent event) {
                    try {
    ...
    private void UserEvent(ActionEvent event) {
                    try {
            Parent root = FXMLLoader.load(getClass().getClassLoader().getResource("edu/fasty/gui/MakeBid.fxml"));
            Scene scene = new Scene(root);
            Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
            stage.setScene(scene);
            stage.show();
        } catch (IOException ex) {
            Logger.getLogger(MakeBidController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
